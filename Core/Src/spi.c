...
{ .GetVersion = NULL, .GetCapabilities = NULL, .Initialize = SPI1_Initialize,
		.Uninitialize = NULL, .PowerControl = NULL, .Send = SPI1_Send,
		.Receive = SPI1_Receive, .Transfer = SPI1_Transfer,
		.GetDataCount = NULL, .Control = SPI1_Control, .GetStatus = NULL, };

int32_t spi_master_init(spi_ctrl_t *spi, SPI_HandleTypeDef *handle,
		ARM_SPI_SignalEvent_t cb_event);
...
int32_t spi_master_transfer(spi_ctrl_t *spi, uint8_t *data_out, uint8_t *data_in, uint32_t num);
int32_t SPI1_Initialize(ARM_SPI_SignalEvent_t cb_event)
{
	...
	if (ret == ARM_DRIVER_OK)
	{
		...
		HAL_GPIO_WritePin(SPI1_Ctrl.cs.GPIO, SPI1_Ctrl.cs.GPIO_Pin,
				SPI1_Ctrl.cs.inactive);
	}
...
int32_t SPI1_Transfer(const void *data_out, void *data_in, uint32_t num)
{
	return spi_master_transfer(&SPI1_Ctrl, (uint8_t *)data_out,
			(uint8_t *)data_in, num);
}
...
int32_t spi_master_send(spi_ctrl_t *spi, const uint8_t *pData, uint32_t size)
{
	...
	if (HAL_OK
			== HAL_SPI_Transmit_IT(spi->handle, (const uint8_t*) pData,
					(uint16_t) size))
	{
		flags = osEventFlagsWait(spi->event, INT32_MAX, osFlagsWaitAny,
				osWaitForever);
		if (flags <= INT32_MAX)
		{
			//
...
int32_t spi_master_receive(spi_ctrl_t *spi, uint8_t *pData, uint32_t size)
{
	...
	ret = (int32_t) HAL_SPI_Receive_IT(spi->handle, (uint8_t*) pData,
			(uint16_t) size);

	return ret;
}

int32_t spi_master_transfer(spi_ctrl_t *spi, uint8_t *data_out,
		uint8_t *data_in, uint32_t num)
{
	int32_t ret = (int32_t)ARM_DRIVER_OK;
	uint32_t flags = 0;

	if(HAL_OK == (int32_t)HAL_SPI_TransmitReceive_IT(spi->handle, data_out, data_in, (int16_t)num))
	{
